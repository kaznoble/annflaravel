...
class StaffuserController extends BaseController {
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['groupid'] = Session::get('gid');
		...
		if ($validator->passes()) {
			...
			else
			{
				if( !empty($newpassword) )
					...
					$data['password'] = Input::get('hid_password');
			}
			...
			if( Input::get('id') =='')
			{
				$staffNo = substr('000000000' . $ID, -8);
				$staffNo = 'SN' . $staffNo;
		    	DB::table('tb_users')->where('id', $ID)->update(array('staff_id' => $staffNo));
			...
			// Redirect after save	
			return Redirect::to('Staffuser')->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			return Redirect::to('Staffuser/add/'.$id)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
