...
class BaseController extends Controller {
	...
	public function __construct() {
		...
		if(Input::get('type') == 'menu' || Input::get('customer') == 'new')
		{
    ...
    // Pagination Function for start and end date search (Submission report and history report)
    function injectPaginate_submissionreport()
    {
        $submission_start_date 	= (!is_null(Input::get('submission_start_date')) ? Input::get('submission_start_date') : '');
        $submission_end_date 	= (!is_null(Input::get('submission_end_date')) ? Input::get('submission_end_date') : '');
        $SubmissionID = (!is_null(Input::get('SubmissionID')) ? Input::get('SubmissionID') : '');
        $sort 	= (!is_null(Input::get('sort')) ? Input::get('sort') : '');
        $order 	= (!is_null(Input::get('order')) ? Input::get('order') : '');
        $rows 	= (!is_null(Input::get('rows')) ? Input::get('rows') : '');
        $search 	= (!is_null(Input::get('search')) ? Input::get('search') : '');
        $masterDetail 	= (!is_null(Input::get('md')) ? Input::get('md') : '');
        $appends = array();
        if($submission_start_date!='') 	$appends['submission_start_date'] = $submission_start_date;
        if($submission_end_date!='') 	$appends['submission_end_date'] = $submission_end_date;
        if($SubmissionID)   $appends['SubmissionID'] = $SubmissionID;
        if($sort!='') 	$appends['sort'] = $sort;
        if($sort!='') 	$appends['sort'] = $sort;
        if($order!='') 	$appends['order'] = $order;
        if($rows!='') 	$appends['rows'] = $rows;
