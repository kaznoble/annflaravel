            ...
            @foreach ($rowData as $row)
                <tr>
					 ...
					 <td>					 
						...
						@else	
							...
							@if($field['field'] == 'date_of_birth')
								{{ date("d/m/Y", strtotime(SiteHelpers::gridDisplay($row->$field['field'],$field['field'],$conn))) }}								
							@else
				 ...
				 <td>
					...
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('Customerdetails/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
