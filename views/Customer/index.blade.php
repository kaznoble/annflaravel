	  ...
	  <ul class="breadcrumb-buttons collapse">
        <li class="dropdown"><a href="" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-search3"></i> <span>Search</span> <b class="caret"></b></a>
          <div class="popup dropdown-menu dropdown-menu-right">
			...
			<li><a href="{{ URL::to('Customer/download') }}" class="tips" title="Download"><i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/Customer') }}" class="tips"  title="Configuration"><i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('Customer/add') }}" class="tips"  title="Create"><i class="icon-plus-circle2"></i>&nbsp;</a></li>
			@endif  
    ...
    <table class="table table-striped ">
        ...
        <tbody>
            @foreach ($rowData as $row)
                <tr>
					<td width="50"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="{{ $row->customer_no }}" />  </td>				
                    <td width="75">
				<div class="btn-group">
                  <button class="btn btn-success btn-xs dropdown-toggle" data-toggle="dropdown">
				  	<i class="icon-cogs"></i> <span class="caret"></span>
				</button>
				{{--*/ $id = SiteHelpers::encryptID($row->customer_no) /*--}}
				<ul style="display: none;" class="dropdown-menu icons-right">
					@if($access['is_detail'] ==1)
					<li><a href="{{ URL::to('Customer/show/'.$id)}}"><i class="icon-cogs"></i> View Detail </a></li>
					@endif
					@if($access['is_edit'] ==1)
					<li><a href="{{ URL::to('Customer/add/'.$id)}}"><i class="icon-grid3"></i> Edit</a></li>
					@endif
	...
	<div class="row">
	 <div class="col-sm-5">
	  <div class="table-actions">
	   ...
	   {{ Form::open(array('url'=>'Customer/filter/')) }}
