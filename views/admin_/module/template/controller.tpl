...
class {controller}Controller extends BaseController {
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$this->model->insertRow($data , Input::get('{key}'));

			return Redirect::to('{class}')->with('message', SiteHelpers::alert('success','Data Has Been Save Successfull'));
		} else {
			return Redirect::to('{class}/add/'.$id)->with('message', SiteHelpers::alert('error','The following errors occurred'))
