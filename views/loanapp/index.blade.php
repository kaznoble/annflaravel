...
<div class="page-content">
	...
	<div class="breadcrumb-line">
		...
		<ul class="breadcrumb-buttons collapse">
			@if($access['is_excel'] ==1)
				<li><a href="{{ URL::to('loanapp/download') }}?sort={{ (Input::get('sort')?Input::get('sort'):'app_primary_id') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
						<i class="icon-folder-download2"></i>&nbsp; </a></li>
			@endif
			@if(Session::get('gid') ==1)
				<li><a href="{{ URL::to('module/config/loanapp') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
						<i class="icon-cog"></i>&nbsp; </a></li>
			@endif
			@if($access['is_add'] ==1)
				<li><a href="{{ URL::to('loanapp/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
	...
	<ul class="nav nav-tabs">
		<li class="active"><a data-toggle="tab" style="background-color:#bad6c1;" href="{{ url('/loanapp') }}">Loan Application</a></li>
		<li><a href="{{ url('/loanapphistory') }}" style="background-color:#ddd4c1;">Loan Application History</a></li>
	</ul>
		...
		<table class="table table-striped ">
			...
			@foreach ($rowData as $row)
				<tr style="background-color:{{ ($row->admin_view == 0 ? '#d6eab4' : '') }};" >
						...
						<?php } else { ?>
							<td>
								...
								@elseif($field['field'] == 'successful_applicant')
									@if($row->admin_view == 1)
										{{ Form::select('successful_applicant', ([''=>'--select--','0'=>'No','1'=>'Yes']), SiteHelpers::gridDisplay($row->{$field['field']},$field['field'],$conn), ['class' => 'successful_applicant_change', 'data-item' => $row->app_primary_id]) }}
									@else
										@if($row->successful_applicant == '')
													
										@elseif($row->successful_applicant == 1)
											Yes
										@elseif($row->successful_applicant == 0)
											No
										@endif
									@endif																	
								@elseif($field['field'] == 'processed')
									@if($row->admin_view == 1)
										{{ Form::select('processed', (['0'=>'No','1'=>'Yes']), SiteHelpers::gridDisplay($row->{$field['field']},$field['field'],$conn), ['class' => 'processed_change', 'data-item' => $row->app_primary_id]) }}
									@else
										No
									@endif								
								...
								@elseif($field['field'] == 'timestamp')
									...
									@endif
                                @elseif($field['field'] == 'reason' && $row->successful_applicant == '0' && $row->admin_view == 1)
					...
					<td>
						<div class="btn-group">
							{{ Form::select('admin_view', (['0'=>'New','1'=>'Seen']), ($row->admin_view == 0)?0:1, ['class' => 'change_admin_view', 'data-item' => $row->app_primary_id]) }}
							{{--*/ $id = SiteHelpers::encryptID($row->app_primary_id) /*--}}
							@if($access['is_edit'] ==1)
								<a  href="{{ URL::to('loanapp/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
							@endif
...
<script>
    ...
    $(document).ready(function(){
        $('.do-quick-search').click(function(){
            $('#SximoTable').attr('action','{{ URL::to("loanapp/multisearch")}}');
