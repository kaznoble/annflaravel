  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('searchcustomer') }}">{{ $pageTitle }}</a></li>
	...
	<div class="panel-default panel">
		...
		<ul class="parsley-error-list">
		 ...
		 {{ Form::open(array('url'=>'searchcustomer/save/'.SiteHelpers::encryptID($row['id']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
				<div class="col-md-12">
						<fieldset><legend> SearchCustomer</legend>
							...
							class='select2 '    ></select> 
								  ...
								  <div class="form-group  " >
									<label for="Time Address" class=" control-label col-md-4 text-left"> Time Address </label>
									<div class="col-md-8">
									  {{ Form::text('time_address', $row['time_address'],array('class'=>'form-control', 'placeholder'=>'',   )) }} 
									 </div> 
								  </div> 					
								  <div class="form-group  " >
								  ...
								  <div class="form-group  " >
									<label for="Previous Time Address" class=" control-label col-md-4 text-left"> Previous Time Address </label>
									<div class="col-md-8">
									  {{ Form::text('previous_time_address', $row['previous_time_address'],array('class'=>'form-control', 'placeholder'=>'',   )) }} 
									 </div> 
								  </div> 					
								  <div class="form-group  " >
							...
							class='select2 '    ></select> 
								  ...
								  <div class="form-group  " >
									<label for="Employment Phone" class=" control-label col-md-4 text-left"> Employment Phone </label>
									<div class="col-md-8">
									  {{ Form::text('employment_phone', $row['employment_phone'],array('class'=>'form-control', 'placeholder'=>'',   )) }} 
									 </div> 
								  </div> 					
								  <div class="form-group  " >
			...
			<div style="clear:both"></div>	
			  ...
			  <div class="form-group">
				...
				<button type="button" onclick="location.href='{{ URL::to('searchcustomer') }}' " id="submit" class="btn btn-success ">  {{ Lang::get('core.sb_cancel') }} </button>
				</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#customer_no").jCombo("{{ URL::to('searchcustomer/comboselect?filter=customer_main:customer_no:customer_no') }}",
		{  selected_value : '{{ $row["customer_no"] }}' });
		
		$("#user_id").jCombo("{{ URL::to('searchcustomer/comboselect?filter=users:id:id') }}",
		{  selected_value : '{{ $row["user_id"] }}' });
		
		$("#title").jCombo("{{ URL::to('searchcustomer/comboselect?filter=customer_title:id:id') }}",
		{  selected_value : '{{ $row["title"] }}' });
		
		$("#marital_status").jCombo("{{ URL::to('searchcustomer/comboselect?filter=marital_status:id:id') }}",
		{  selected_value : '{{ $row["marital_status"] }}' });
		
		$("#employment_status").jCombo("{{ URL::to('searchcustomer/comboselect?filter=employment_status:id:id') }}",
		{  selected_value : '{{ $row["employment_status"] }}' });
		
		$("#cus_residential_status").jCombo("{{ URL::to('searchcustomer/comboselect?filter=residential_status:id:id') }}",
