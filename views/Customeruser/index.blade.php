  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('Customeruser/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/Customeruser') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1 && FALSE)
	   		<li><a href="{{ URL::to('Customeruser/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
				...
				@foreach ($tableGrid as $t)
					...
					<td {{ ($t['field'] == 'customer_no' ? 'class="td_customer_no"' : '') }} >						
						@if( $t['field'] == 'customer_no' )
							<input class="form-control input-sm" type="text" name="customer_no" value="" placeholder="C#########" >
						@else
            ...
            @foreach ($rowData as $row)
                <tr>
					 ...
					 <td>					 
						...
						@else	
							...
							@if($field['field'] == 'created_at' || $field['field'] == 'updated_at')
								{{ date("d/m/Y H:i:s", strtotime(SiteHelpers::gridDisplay($row->$field['field'],$field['field'],$conn))) }}	
							@else
				 ...
				 <td>
					...
					<a href="{{ URL::to('Customeruser/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('Customeruser/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
				...
				@include('Customeruser.inlineview')
            @endforeach
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("Customeruser/multisearch")}}');
		$('#SximoTable').submit();
	});
