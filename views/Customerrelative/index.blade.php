  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('Customerrelative/download') }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/Customerrelative') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1 && FALSE)
	   		<li><a href="{{ URL::to('Customerrelative/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
    ...
    <table class="table table-striped ">
        <thead>
			<tr id="sximo-quick-search" >
				...
				@foreach ($tableGrid as $t)
					@if($t['view'] =='1' && $t['field'] != 'cust_rel_id' && $t['field'] != 'user_id')
        ...
        <tbody>
            ...
            @foreach ($rowData as $row)
                <tr>
					<td width="50"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="{{ $row->cust_rel_id }}" />  </td>	
					@if($access['is_detail'] ==1) <td >
						<a href="javascript:void(0)" class="expand-row" id="{{ SiteHelpers::encryptID($row->cust_rel_id) }}">
						<i class="icon-plus"></i></a>
					</td>	 
					@endif								
				 @foreach ($tableGrid as $field)
					 @if($field['view'] =='1' && $field['field'] != 'cust_rel_id' && $field['field'] != 'user_id')
					 <td>					 
				 ...
				 <td>
				 	<div class="btn-group">
					{{--*/ $id = SiteHelpers::encryptID($row->cust_rel_id) /*--}}
				 	@if($access['is_detail'] ==1)
					<a href="{{ URL::to('Customerrelative/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('Customerrelative/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
				...
				@include('Customerrelative.inlineview')
            @endforeach
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("Customerrelative/multisearch")}}');
		$('#SximoTable').submit();
	});
