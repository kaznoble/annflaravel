        ...
        <thead>
			...
			<tr id="sximo-quick-search" >
				...
				@if($access['is_detail'] ==1)
					<td>
						<input type="checkbox" class="checkall" /><br />Select ALL accounts.
					</td>
				@endif
				<td> </td>
        ...
        <tbody>
			<tr>
				...
				@if(Session::get('gid') ==1 || Session::get('gid') ==2)
					<br /><a href="javascript://ajax"  onclick="SximoDeleteRow();" class="tips" title="{{ Lang::get('core.btn_remove') }}">[DELETE]</a>				
					{{ Form::open(array('url'=>'Customeraccounts/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTableRow' )) }}
						{{ Form::hidden('checkedAccId','') }}				
						{{ Form::hidden('customer_no', $customer_no) }}										
					{{ Form::close() }}
				@endif
				</th>
            ...
            @foreach ($rowData as $row)
                <tr>
					<!--<td width="50"> {{ ++$i }} </td>-->
					<td width="50"><input type="checkbox" class="ids" name="accid" value="{{ $row->account_id }}" data-account_no="{{ $row->account_no }}" data-customer_no="{{ $row->customer_no }}" data-payment-type="{{ $row->payment_type }}" />  </td>	
...
$(document).ready(function(){
    ...
    $('.checkall').change(function() {
        if($(this).is(":checked")) {
			$('body').append('<div id="dataConfirmModal" class="modal" role="dialog" aria-labelledby="dataConfirmLabel" aria-hidden="true"><div class="modal-header" style="border:none;" ><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="dataConfirmLabel">Title</h3></div><div class="modal-body" style="background-color:#fff;width:50%;margin-left:auto;margin-right:auto;text-align:center;font-size:1.1em;font-weight:bold;margin-top:15%;" >Are you sure that you want to delete all accounts from the list?</div><div class="modal-footer" style="text-align:center;border:none;"><button class="btn btn-success" data-dismiss="modal" aria-hidden="true">No</button><button class="btn btn-danger" data-dismiss="modal" aria-hidden="true" id="dataConfirmOK">Yes</button></div></div>');
			
			jQuery('#dataConfirmModal').find('.modal-body').text('IMPORTANT! Are you sure that you want to SELECT all accounts from the list?');
            jQuery('.modal-footer .btn-success').attr('onClick', 'javascript:confirmNo();');
			jQuery('#dataConfirmOK').attr('onClick', 'javascript:closeModal();');
	...
	$('.update_card_submit').click(function(e) {
        ...
        if (window.confirm("Are you sure?")) {
            ...
            location.href = "{{ $baseurl }}/realpayment/realauth_cardupdate.php?_token=<?php echo csrf_token(); ?>&account_no="+accountno+"&customer_no="+customerno+"&id="+account_id+"&card_name="+card_name+"&card_number="+card_number+"&card_expiry="+card_expiry+"&card_type="+card_type;
        }
	});			
...
function closeModal()
{
	jQuery('#dataConfirmModal').modal({show:false});
}
...
function SximoDeleteRow( rec_id )
{
	var checkedAccId = $("input[name='accid']:checked").map(function() {return this.value;}).get().join(',');
	$("input[name='checkedAccId']").val(checkedAccId);
	...
	if(total <=0)
	{
		...
		{
				$('#SximoTableRow').submit();// do the rest here	
		}
	}
	...
	if( rec_id > 0 )
	{
		if(confirm('Are you sure removing selected rows ?'))
		{
				$('#delete_form_' + rec_id).submit();// do the rest here	
		}	
	}
