{{--*/ usort($tableGrid, "SiteHelpers::_sort") /*--}}
  <div class="page-content">
        ...
        <tbody>
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <!-- check load status -->
					 @if($field['view'] =='1')
					 <td @if($row->loan_status == '8' && ($row->payment_type == "Cash" || $row->payment_type == "Auto")) style="background-color:#d67b7b";
                          @elseif($row->loan_status == '1' && $row->payment_type == "Cash"  ) style="background-color:#97E2F7;"
                          @elseif($row->loan_status == '1') style="background-color:#CAEDBE;"
	...
	@if( !empty($_GET['search']) && $maxloansallowed )
		...
		<a href="{{ URL::to( 'Customeraccounts/add/?cu='.Session::get('session_id').'&ur='.SiteHelpers::encryptID($userID).'&form=new' )}}" class="btn btn-info" >ADD Account</a>	
	@endif
...
$(document).ready(function(){
	...
	$('.add_cash_submit').click(function(e) {
		e.preventDefault();
            ...
            location.href = "{{ $baseurl }}/Cashpayment/add/" + account_id;
