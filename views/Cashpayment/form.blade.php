  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('Cashpayment') }}">{{ $pageTitle }}</a></li>
	...
	<div class="panel-default panel">
		...
		<ul class="parsley-error-list">
		 ...
		 {{ Form::open(array('url'=>'Cashpayment/save/'.SiteHelpers::encryptID($row['account_id']),'id'=>'form_cashpayment', 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
				<div class="col-md-12">
						<fieldset><legend> Cashpayment</legend>
						...
						<div class="form-group {{( $row['no_miss_payment'] > 0 ? '' : 'div_hide' )}}" >
						...
						<div class="form-group change-next-payment " >
							...
							<div class="col-md-8">
								...
								<span class="require_style" >Please remember to set the correct next payment date.</span>
							</div> 
						</div>						
						...
						<div class="form-group  " >
							...
							<div class="col-md-8">
								£{{ Form::text('total_paid_payment', '0', array('class'=>'form-control', 'required' ,'placeholder'=>'', 'style'=>'width:60px;' )) }} 
							</div> 
						</div>	
			...
			<div style="clear:both"></div>	
			  ...
			  <div class="form-group">
				...
				<button type="submit" name="miss_payment" class="btn btn-warning miss-button" value="miss_payment" >  {{ Lang::get('core.btn_missedpayment') }} </button>
				<input type="hidden" name="account_no" id="account_no" value="{{ $row->account_no }}" />
				<input type="hidden" name="customer_no" id="customer_no" value="{{ $row->customer_no }}" />	
				<input type="hidden" name="today_date" id="today_date" value="{{ $todayDate }}" />
				</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.save-button').click(function() {
			...
			if( $('input[name="total_paid_payment"]').val() != '0' )				
			{
			...
			{
				if($('input[name="miss_payment_pay_total"]').val() == '0' ){
					alert('No total value?');
					return false;
			  	}else{
					alert('No Total cash payment value?');
					return false;
				}
			}
			
			// check next payment date to today or future
			var expireDateStr = $('input[name="change_next_payment_due_date"]').val();
			var expireDateArr = expireDateStr.split("/");
			var expireDate = new Date(expireDateArr[2], --expireDateArr[1], expireDateArr[0], 23, 59, 00);
			var todayDate = new Date();			

			if (todayDate > expireDate) {
			   alert('The change next payment due date must be today or later.');
			   return false;
			};
		});
		
		  $(".miss-button").on("click", function(e) {
		  	/*if($('input[name="total_paid_payment"]').val() == '0' ){
		  		alert('No Total cash payment value?');
				return false;
		  	}*/
	        if( !confirm('Are you sure that you want to submit the missing payment?') ) 
	            e.preventDefault();		  	
		  });
