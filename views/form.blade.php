  ...
  <div class="page-content">
						...
						<fieldset><legend> Add/Edit Customer Accounts</legend>
							...
							class='select2 '    ></select> 
								  ...
								  <div class="form-group  " >
									...
									<div class="col-md-8">
									  <select name='frequency' rows='5' id='frequency' code='{$frequency}' 
							class='select2 '    ></select> 
									 </div> 
								  </div> 										  			
								  <div class="form-group  " >
								  ...
								  <div class="form-group  " >
									...
									<div class="col-md-8">
										@if( !empty($row['total_paid_to_date']) )
									  		{{ Form::text('total_paid_to_date', $row['total_paid_to_date'],array('class'=>'form-control', 'placeholder'=>'', 'readonly'=>'true' )) }} 
									  	@else
								  ...
								  <div class="form-group  " >
									...
									<div class="col-md-8">
									  {{ Form::text('early_settlement_balance', date('d/m/Y', strtotime($row['early_settlement_balance'])),array('class'=>'form-control', 'placeholder'=>'', 'readonly'=>'true'   )) }} 
									 </div> 
								  </div> 			
								  <div class="form-group  " >
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('#loan_period, #frequency').change(function(e) {
			loanCalculate();
		});	
		$('input[name="loan_start_date"]').keyup(function() {
			loanCalculate();
		});	
		...
		function loanCalculate() {
			...
			firstPaymentDate = new Date(startLoanDate.setDate(startLoanDate.getDate()+7));		
			endLoanDate = new Date(endLoanDate.setDate(firstPaymentDate.getDate()+loan_week_period));						
			endLoanDate = endLoanDate.toLocaleString(endLoanDate)
			firstPaymentDate = firstPaymentDate.toLocaleString(firstPaymentDate);
			$('input[name="loan_end_date"]').val(endLoanDate.substring(0,10));
			$('input[name="first_payment"]').val(firstPaymentDate.substring(0,10));
			...
			$.get('../../getapr', {periodLoanVal: periodLoanVal, frequencyVal: frequencyVal}, function(data){
				...
				$('input[name="interest_payable"]').val(aprArray[0]);
			});
			/*var base_url = "http://localhost/annadmin";
			$.ajax({
				url: base_url+'/ajax/systemconfig.php',
				type: 'POST',
				data: dataString,
		...
		{  selected_value : '{{ $row["week_loan_period"] }}' });
