  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
      <ul class="breadcrumb">
		...
		<li><a href="{{ URL::to('Merchantpayment') }}">{{ $pageTitle }}</a></li>
	...
	<div class="panel-default panel">
		...
		<ul class="parsley-error-list">
		 ...
		 {{ Form::open(array('url'=>'Merchantpayment/save/'.SiteHelpers::encryptID($row['account_id']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) }}
				<div class="col-md-12">
						<fieldset><legend> Merchant Payment</legend>
			...
			<div style="clear:both"></div>	
			  ...
			  <div class="form-group">
				...
				<button type="button" onclick="location.href='{{ URL::to('Merchantpayment') }}' " id="submit" class="btn btn-success ">  {{ Lang::get('core.sb_cancel') }} </button>
				</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#customer_no").jCombo("{{ URL::to('Merchantpayment/comboselect?filter=customer_main:customer_no:customer_no') }}",
		{  selected_value : '{{ $row["customer_no"] }}' });
		
		$("#user_id").jCombo("{{ URL::to('Merchantpayment/comboselect?filter=users:id:id') }}",
		{  selected_value : '{{ $row["user_id"] }}' });
		
		$("#loan_status").jCombo("{{ URL::to('Merchantpayment/comboselect?filter=loan_status:id:id') }}",
