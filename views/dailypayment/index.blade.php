  ...
  <div class="page-content">
    ...
    <div class="breadcrumb-line">
	  ...
	  <ul class="breadcrumb-buttons collapse">
			...
			<li><a href="{{ URL::to('dailypayment/download?search=') . $searchQuery }}" class="tips" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/dailypayment') }}" class="tips"  title="{{ Lang::get('core.btn_config') }}">
			<i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('dailypayment/add') }}" class="tips"  title="{{ Lang::get('core.btn_create') }}">
        ...
        <thead>
			<tr id="sximo-quick-search" >
				...
				@foreach ($tableGrid as $t)
					@if($t['view'] =='1' )
					<td>				
						@if($t['field'] !== 'account_no' && $t['field'] !== 'customer_no' && $t['field'] !== 'forename' && $t['field'] !== 'surname' && $t['field'] !== 'payment')
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					<a href="{{ URL::to('dailypayment/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="icon-paragraph-justify"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('dailypayment/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="icon-pencil4"></i></a>
					@endif
				...
				@include('dailypayment.inlineview')
            @endforeach
...
$(document).ready(function(){
		...
		$('input[name="next_payment_due_date"]').datepicker({format: 'yyyy-mm-dd', autoclose: true});

	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("dailypayment/multisearch")}}');
		$('#SximoTable').submit();
	});
